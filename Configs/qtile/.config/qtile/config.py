...
def set_layouts():
    return [
        layout.MonadTall(
            border_focus=color['active'], border_width=2, margin=8),
        layout.Max(),
    ]
...
def set_widgets():
    ...
    return [
        ...
        widget.Memory(background=color['inactive']),
        # Date and time widget
        arrow_widget('light'),
        widget.Image(filename='~/.config/qtile/icons/calendarlight.png'),
        widget.Clock(
            background=color['active'], foreground=color['foreground'], format='%A %Y/%m/%d %H:%M'),
        # System tray
        arrow_widget('dark'),
